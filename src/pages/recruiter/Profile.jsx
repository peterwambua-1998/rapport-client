...
const Profile = () => {
  ...
  const { name, email, recruiterProfile } = userProfile;
  const {
    Company = [],
    company_name = "N/A",
    country = "N/A",
  ...
  const coverPhoto = cover_photo_url
    ? getImageUrl(cover_photo_url)
    : backgroundImage;
  const profilePhoto = profile_image_url
    ? getImageUrl(profile_image_url)
    : profilePic;
      ...
      <div className="max-w-4xl w-full bg-white rounded-xl shadow-md overflow-hidden">
        ...
        <div className="mt-14 px-6 ">
              ...
              {company_true && (
                is_verified ? (
                  <div className="flex items-center bg-blue-100 p-1 rounded-full space-x-2">
                    <MdVerified className="text-blue-500 text-xl w-4 h-4" />
                    <span className="text-xs text-blue-600 font-bold">Verified Company</span>
                  </div>
                ) : (
                  <div className="flex items-center bg-gray-100 p-1 rounded-full space-x-2">
                    <MdVerified className="text-gray-500 text-xl w-4 h-4" />
                    <span className="text-xs text-gray-600 font-bold">Not Verified</span>
                  </div>
                )
              )}
            </div>
          ...
          <p className="text-sm text-gray-600">
            {specialization} at {company_name}
          </p>
